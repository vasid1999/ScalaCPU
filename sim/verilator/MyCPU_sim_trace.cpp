...
// tracing object; used in signal handler, hence global
VerilatedVcdC *m_trace;

void sigint_handler(int key){
	if(m_trace){
		m_trace->close();
		m_trace = NULL;
	}
...
int main(int argc,char **argv){
	...
	// TRACING - create new tracing object, assign it to current module and open dump file
	Verilated::traceEverOn(true);
	m_trace = new VerilatedVcdC;
	vmodule->m->trace(m_trace,TRACE_INIT);
	m_trace->open(VCD_DUMPFILE);
...
NEW: Tracing
After every eval(), dump the contents of the module into a trace file
*/
